...
export class SecurePeerKey {
  /**
     ...
     */
  public static async create (seed?: Uint8Array | string): Promise<SecurePeerKey> {
    await sodium.ready
    // if seed is just a simple string password, convert it to full 32 byte seed value
    if (typeof seed === 'string') { seed = sodium.crypto_generichash(32, seed) }
  ...
  initiateHandshake (
    peerId: string
  ): { sharedSecret: Uint8Array, handshake: Handshake } {
    ...
    const handshake: Handshake = {
      ...
      publicSignKey: sodium.to_base64(this.securePeerKeySet.signKeyPair.publicKey),
